...
import { Solucao, Noticias, Prolema, Edital, Ranking, Documentacao, Empresa, User, Equipe, Logado } from "../../../typings";
...
export function InfoWrapper({ children }: { children: React.ReactNode }) {
  ...
  const [logado, setLogado] = useState<Logado>({id:null, isLogado:false});
  const [revalidate, setRevalidate] = useState('');
  const [currentPage, setCurrentPage] = useState(['']);
  const [currentPageShown, setCurrentPageShown] = useState(['']);
  ...
  useEffect(() => {
    ...
    else if (revalidate === 'machine') {
    } 
    else if (revalidate === 'program') {
    } 
    else if (revalidate === 'reward') {
    }
  }, [revalidate]);
